...
In the example above, note the use of the `style` prop on some of the `HeadCell` components. You can get creative with setting `width`, `maxWidth`, or `minWidth` on a header cell, row cell, or its content (EG: a nested `Text` component). You can even combine the `clampLines` prop of `<Text>` to implement ellipsis text overflow.

If you have a cell that contains something like a `Button` and you'd like the cell width to match the `Button` width exactly, you can set the column's corresponding `HeadCell` width to something really small (EG: `1px`). Due to how `<td>` and `<th>` elements determine their width, this will give us the desired result:
...
```tsx
import { useTable } from '@nearai/ui';
